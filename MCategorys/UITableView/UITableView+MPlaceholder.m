...
@implementation UITableView (MPlaceholder)

+ (void)load{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        SEL reloadData = @selector(reloadData);
        SEL m_reloadData = @selector(m_reloadData);
        [self m_exchangeInstanceWithOriginalSelector:reloadData swizzledSelecor:m_reloadData];
...
-(void)m_reloadData{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        //在主线程创建 UI
        [self m_isHavingData:havingData];
    });
 ...
 */
- (void)m_isHavingData:(BOOL)havingData{
    //判断图片响应代理
    ...
    if (isImg || isMsg || isView) {
        ...
        if (height + 50 > Y) {
            ...
            //如果创建了自定义的加载图
            if (isView) {
                self.tableFooterView = [self.delegate performSelector:@selector(m_noDataView)];
                return ;
            }
            //创建视图
            UIImage  *img   = nil;
            NSString *msg   = @"暂无数据";
            UIColor  *color = [UIColor m_colorWithHex:0x9daabf];;
...
-(UIView *)createFooterView:(UIImage *)image message:(NSString *)message color:(UIColor *)color{
    ...
    label.textColor = color ? color : [UIColor m_colorWithHex:0x9daabf];
    label.text = message;
